...
using namespace std;
#define maxpos 15
#define maxndaugh 32768 //524288	// pow(2,maxpos-1)
 ...
 *
 * Read in sequence length N, mean number of crossovers per sequence
 * lambda, library size L, and positions of variable base-pairs (codons)
 * A[i], i=1,M. Calculate the probabilities P(b_i=0), P(B_i=1) of there
 * being an even or odd number respectively of crossovers between
 * consecutive variable base-pairs A[i] and A[i+1].  Calculate the
 * relative probabilities of each of the 2^M possible daughter sequences
 * and the probability that each daughter sequence will be present in the
 * library.  Sum these probabilities to find the expected number of
 * distinct sequences in the library.
 *
 * Input lambda can be the true underlying crossover rate (xtrue = 1) or
 * the mean number of observable crossovers (xtrue = 0).
 *
 * Also outputs file driver.dat (probability for each daughter sequence)
 *
 * Also outputs the observed lambda for a given input true lambda.  The
 * user should try different input lambda values until the known observed
 * lambda is reproduced.
 */

// Function declarations
double          calcprob(double lambda, int N, int M, int nn[maxpos],
			 int A[maxpos], double lnn[maxpos],
			 double Pb0[maxpos], double Pb1[maxpos], int nseqs,
...
main(int argc, char *argv[])
{
	...
	if (i > 0 && A[i] <= A[i - 1]) {
	    cout <<
		"Error: Variable positions must be in numerical order.<br>\n";
	    exit(EXIT_FAILURE);
	}
	if (i > 0 && A[i] == A[i - 1] + 1) {	// Adjacent variable
						// positions
	    Meff -= 1;
	    cout << "Warning: Variable positions " << A[i -
							1] << " and " <<
		A[i]
	...
	if (lambda > float (Meff - 1) / 2) {
	    cout <<
		"Error: Mean number of <i>observable</i> crossovers can't "
		<<
		"exceed '0.5 x (number of non-adjacent variable positions - 1)' "
		<< "= " << float (Meff -
				  1) /
		2 << ".  " << "You entered " << lambda <<
	...
	while (abs(diff) > tolerance) {
	    if (iter > maxiter) {
		cout <<
		    "Error: failed to converge on true crossover rate in "
	    ...
	    lobsm =
		calcprob(lambdam, N, M, nn, A, lnn, Pb0, Pb1, nseqs, PBk);
	    if (lobsm > lobsin) {
		lobs2 = lobsm;
		lambda2 = lambdam;
	    } else {
    ...
    cout << "Total number of possible sequences = " << 2 *
	nseqs << ".<br>\n";
...
double
calcprob(double lambda, int N, int M, int nn[maxpos], int A[maxpos],
	 ...
	 int nseqs, double PBk[maxndaugh])
{
    ...
    // Calculate probabilities P(b_i=0) and P(b_i=1) for an even or odd
    // number
    // of crossovers between consecutive varying base-pairs A[i] and
    // A[i+1].
    for (i = 0; i < M - 1; ++i) {
    ...
    // Encode possible daughter sequences as binary sequences (1 = odd
    // number of
	...
	// PBk[k]*0.5 since each binary sequence corresponds to two
	// `inverse'
	// daughter sequences
	// Calculate lambda^obs - the mean number of _observable_
	// crossovers
	// per sequence
